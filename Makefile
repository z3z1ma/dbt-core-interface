...
everything: .venv-dbt10/bin/python .venv-dbt11/bin/python .venv-dbt12/bin/python .venv-dbt13/bin/python .venv-dbt14/bin/python .venv-dbt15/bin/python .venv-dbt16/bin/python .venv-dbt17/bin/python .venv-dbt18/bin/python
.PHONY: everything
...
.venv-dbt16/bin/python:
	...
	.venv-dbt15/bin/pip install pytest WebTest .
	for adapter in $(ADAPTERS); do \
		.venv-dbt15/bin/pip install "dbt-$$adapter>=1.5.0,<1.7.0"; \
		.venv-dbt15/bin/pip install "dbt-core>=1.6.0,<1.7.0"; \
	done
...
.venv-dbt17/bin/python:
	...
	.venv-dbt15/bin/pip install pytest WebTest .
	for adapter in $(ADAPTERS); do \
		.venv-dbt15/bin/pip install "dbt-$$adapter>=1.7.0,<1.8.0"; \
		.venv-dbt15/bin/pip install "dbt-core>=1.7.0,<1.8.0"; \
	done
...
.venv-dbt18/bin/python:
	...
	.venv-dbt15/bin/pip install pytest WebTest .
	for adapter in $(ADAPTERS); do \
		.venv-dbt15/bin/pip install "dbt-$$adapter>=1.8.0,<1.9.0"; \
		.venv-dbt15/bin/pip install "dbt-core>=1.8.0,<1.9.0"; \
	done
